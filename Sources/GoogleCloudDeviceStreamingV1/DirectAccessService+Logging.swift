...
extension Clients {
  final class DirectAccessServiceLogging: DirectAccessServiceStub {
    let inner: any DirectAccessServiceStub
    let logger: Logger

    public init(_ inner: any DirectAccessServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-devicestreaming-v1"
      logger[metadataKey: "gcp.client.service"] = "devicestreaming"
      logger[metadataKey: "gcp.experimental.swift.client"] = "DirectAccessService"
      self.inner = inner
      self.logger = logger
    }
