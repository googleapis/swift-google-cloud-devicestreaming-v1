...
extension Clients {
  class DirectAccessServiceTransport: DirectAccessServiceStub {
    ...
    public func createDeviceSession(
      ...
      let path = try { () throws -> Swift.String in
        ...
        return "/v1/\(pathVariable0)/deviceSessions"
      }()
      var query = [
    ...
    public func updateDeviceSession(
      ...
      let path = try { () throws -> Swift.String in
        guard let pathVariable0 = request.deviceSession.map({ $0.name }), !pathVariable0.isEmpty
        else {
          throw GoogleCloudGax.RequestError.binding(
            "'request.device_session.name' is not set or is empty")
        }
        return "/v1/\(pathVariable0)"
      }()
      var query = [
